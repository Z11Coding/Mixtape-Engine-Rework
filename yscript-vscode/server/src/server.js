...
const tokenizer = new tokenizer_1.YScriptTokenizer();
const parser = new parser_1.YScriptParser();
const validator = new validator_1.YScriptValidator();
const completion = new completion_1.YScriptCompletion();
const hover = new hover_1.YScriptHover();
...
connection.onDidChangeConfiguration(change => {
    if (hasConfigurationCapability) {
        ...
        globalSettings = ((change.settings.yscript || defaultSettings));
    }
...
connection.onHover(async (_textDocumentPosition) => {
    const document = documents.get(_textDocumentPosition.textDocument.uri);
    if (!document) {
        return null;
    }
