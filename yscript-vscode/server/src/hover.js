...
exports.YScriptHover = void 0;
const node_1 = require("vscode-languageserver/node");
class YScriptHover {
    getHoverInfo(ast, offset, position) {
    ...
    findNodeAtPosition(node, offset) {
    ...
    getNodeHoverInfo(node) {
        switch (node.type) {
            case 'Identifier':
                return this.getIdentifierHover(node);
            case 'VariableDeclaration':
                return this.getVariableDeclarationHover(node);
            case 'FunctionDeclaration':
                return this.getFunctionDeclarationHover(node);
            case 'ClassDeclaration':
                return this.getClassDeclarationHover(node);
            case 'Literal':
                return this.getLiteralHover(node);
            case 'BinaryExpression':
                return this.getBinaryExpressionHover(node);
            case 'CallExpression':
                return this.getCallExpressionHover(node);
            case 'EmbeddedCode':
                return this.getEmbeddedCodeHover(node);
            case 'TypeExpression':
                return this.getTypeExpressionHover(node);
            default:
                return null;
        }
    }
    getIdentifierHover(node) {
    ...
    getVariableDeclarationHover(node) {
    ...
    getFunctionDeclarationHover(node) {
    ...
    getClassDeclarationHover(node) {
        ...
        if (node.interfaces && node.interfaces.length > 0) {
            const interfaces = node.interfaces.map((i) => i.name).join(', ');
        ...
        if (node.body?.body) {
            ...
            const methods = members.filter((m) => m.type === 'FunctionDeclaration').length;
            const fields = members.filter((m) => m.type === 'VariableDeclaration').length;
            if (methods > 0 || fields > 0) {
                hover += `\\n\\n**Members:**`;
                if (fields > 0)
                    hover += ` ${fields} field${fields > 1 ? 's' : ''}`;
                if (methods > 0)
                    hover += `${fields > 0 ? ', ' : ' '}${methods} method${methods > 1 ? 's' : ''}`;
            }
        }
        return hover;
    }
    ...
    getBuiltinTypeDocumentation(typeName) {
        const docs = {
...
exports.YScriptHover = YScriptHover;
