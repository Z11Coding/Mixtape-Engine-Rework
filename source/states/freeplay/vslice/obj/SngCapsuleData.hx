	...
	function updateMeta()
	{
		...
		if (meta != null) {
			...
			if (meta != null) { // Mixtape Metadata doesn't have everything P-Slice does, so im gonna have to accomidate where I can
				...
				freeplayPrevStart = 0;
				freeplayPrevEnd = 0.2;
				songStartingBpm = try{backend.Song.getChart(getNativeSongId()+(currentDifficulty.toLowerCase() != "normal" ? currentDifficulty.toLowerCase() : ""), getNativeSongId()).bpm;}catch(e){0;}
