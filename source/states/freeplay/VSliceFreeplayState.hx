...
import states.freeplay.vslice.FreeplayScore;
import states.freeplay.vslice.FreeplaySongData;
import states.freeplay.vslice.FreeplayStyle;
import states.freeplay.vslice.FreeplayStyleRegistry;
import states.freeplay.vslice.IntervalShake;
...
import states.freeplay.vslice.backcards.BackingCard;
import states.freeplay.vslice.backcards.BoyfriendCard;
import states.freeplay.vslice.backcards.LuaCard;
import states.freeplay.vslice.backcards.PicoCard;
import states.freeplay.vslice.capsule.SongCapsuleGroup;
...
class VSliceFreeplayState extends MusicBeatSubstate
{
  ...
  public static final DEFAULT_DOTS_GROUP_POS:Array<Int> = [260, 170];
	...
	override function create():Void
	{
        ...
        //if (ClientPrefs.data.fpCards)
		//{
			switch (currentCharacterId)
			{
				case(ClientPrefs.data.lowQuality) => true:
					backingCard = null;
    ...
    Highscore.reloadModifiers();
		...
		if (currentCharacter?.getFreeplayDJData() != null)
		{
			...
			{
				dj = new FreeplayDJ((CUTOUT_WIDTH * DJ_POS_MULTI), 322, currentCharacter);
				exitMovers.set([dj], {
					x: -dj.width * 1.6,
					speed: 0.5
				});
...
// Check if showcase mode is enabled, not in Archipelago mode, and warnings are enabled
	...
	function rankDisplayNew(fromResults:Null<FromResultsParams>):Void
	{
		...
		new FlxTimer().start(0.1, _ ->
		{
			...
			{
				case SHIT:
					FunkinSound.playOnce('ranks/rankinbad');
				case PERFECT:
					FunkinSound.playOnce('ranks/rankinperfect');
				case PERFECT_GOLD:
					FunkinSound.playOnce('ranks/rankinperfect');
				default:
					FunkinSound.playOnce('ranks/rankinnormal');
			}
			rankCamera.zoom = 1.3;
	...
	function rankAnimSlam(fromResultsParams:Null<FromResultsParams>)
	{
		...
		{
			case SHIT:
				FunkinSound.playOnce('ranks/loss');
			case GOOD:
				FunkinSound.playOnce('ranks/good');
			case GREAT:
				FunkinSound.playOnce('ranks/great');
			case EXCELLENT:
				FunkinSound.playOnce('ranks/excellent');
			case PERFECT:
				FunkinSound.playOnce('ranks/perfect');
			case PERFECT_GOLD:
				FunkinSound.playOnce('ranks/perfect');
			default:
				FunkinSound.playOnce('ranks/loss');
  ...
  function refreshDots(amount:Int, index:Int, prevIndex:Int):Void
  {
	...
	override function update(elapsed:Float):Void
	{
		...
		lerpScore = backend.pslice.MathUtil.smoothLerp(lerpScore, intendedScore, elapsed, 0.5);
		lerpCompletion = backend.pslice.MathUtil.smoothLerp(lerpCompletion, intendedCompletion, elapsed, 0.5);
	...
	/**
	 * Build an instance of `FreeplayState` that is above the `CategoryState`.
	 * @return The CategoryState with the FreeplayState as a substate.
	 */
	public static function build(?params:FreeplayStateParams, ?stickers:StickerSubState):MusicBeatState
	{
		var result = new CategoryState();
		result.openSubState(new VSliceFreeplayState(params, stickers));
