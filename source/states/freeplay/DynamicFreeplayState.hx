...
import objects.DynamicScrollableContainer;
...
class DynamicFreeplayState extends MusicBeatState
{
	public static var instance:DynamicFreeplayState;
	...
	private var dynamicContainer:DynamicScrollableContainer<Alphabet>;
	private var iconContainer:DynamicScrollableContainer<HealthIcon>;
	private var loadingTracker:LoadingStateTracker;
	private var isDynamicLoading:Bool = false;
	private var loadingQueue:Array<Int> = []; // Queue of song indices to load
	private var loadingText:FlxText;
	private var itemsPerFrame:Int = 1;

	// Maps to track items by song index (for radiant loading)
	private var songByIndex:Map<Int, Alphabet> = new Map();
	private var iconByIndex:Map<Int, HealthIcon> = new Map();
	...
	override function create()
	{
		...
		// Check if dynamic loading should be enabled BEFORE reloading
		isDynamicLoading = ClientPrefs.data.dynamicFreeplayLoading != "Off";

		if (isDynamicLoading)
		{
			// Initialize tracker (will be set up in reloadSongs with queue)
			loadingTracker = new LoadingStateTracker();
		}

		fpManager.reloadFreeplay(true);

		// Explicitly call reloadSongs to load the actual song items
		reloadSongs();
	...
	public function reloadSongs(?refresh:Bool = false)
	{
		if (instance != null)
		{
			...
			// Clear index maps for radiant loading
			songByIndex.clear();
			iconByIndex.clear();
	...
	private function createSongItem(i:Int):Void
	{
		if (i < 0 || i >= fpManager.songList.length)
			return;
		...
		if (APInfo.inArchipelagoMode) {
			...
			// For tracking victory song completion
			isMissing = colorState == APFreeplayManager.APSongColorState.VictoryNoChecks;
			someLocationsNotMissing = colorState == APFreeplayManager.APSongColorState.VictoryPartial;
		}
		...
		songText.visible = songText.active = songText.isMenuItem = false;
		songText.alpha = 0.4; // Set transparency for non-selected items
		...
		icon.sprTracker = songText;
		icon.visible = icon.active = false;
		icon.alpha = 0.4; // Set transparency for non-selected items
		iconArray.push(icon);
		iconList.add(icon);

		// Track item by song index for proper culling
		songByIndex.set(i, songText);
		iconByIndex.set(i, icon);

		// Notify tracker that one item has loaded
		if (loadingTracker != null)
			loadingTracker.itemLoaded();
	}
	...
	override function update(elapsed:Float)
	{
		...
		// Continue loading songs one per frame if dynamic loading is enabled
		if (isDynamicLoading && loadingQueue.length > 0)
		{
			for (i in 0...itemsPerFrame)
			{
				if (loadingQueue.length > 0)
				{
					var index = loadingQueue.shift();
			...
			if (loadingText != null)
			{
				var loaded = loadingTracker.getLoadedItems();
				var total = loadingTracker.getTotalItems();
				loadingText.text = 'Loading... $loaded/$total';
			}
		}
		else if (isDynamicLoading && loadingText != null)
		{
	...
	function changeSelection(change:Int = 0, playSound:Bool = true)
	{
		...
		for (i in 0...fpManager.songList.length)
		{
			var icon = iconByIndex.get(i);
			if (icon != null && icon.animation != null && icon.animation.curAnim != null)
			{
				icon.alpha = 0.4;
				switch (icon.type) {
					case SINGLE: icon.animation.curAnim.curFrame = 0;
					case WINNING: icon.animation.curAnim.curFrame = 2;
					default: icon.animation.curAnim.curFrame = 0;
				}
			}

			if (i == curSelected)
			{
				var selectedIcon = iconByIndex.get(curSelected);
				if (selectedIcon != null && selectedIcon.animation != null && selectedIcon.animation.curAnim != null)
				{
					selectedIcon.alpha = 1;
					switch (selectedIcon.type) {
						case SINGLE: selectedIcon.animation.curAnim.curFrame = 0;
						case WINNING: selectedIcon.animation.curAnim.curFrame = 1;
						default: selectedIcon.animation.curAnim.curFrame = 1;
					}
				}
			}
		}
	...
	public function updateTexts(elapsed:Float = 0.0)
	{
		...
		{
			var songItem = songByIndex.get(i);
			if (songItem != null) songItem.visible = songItem.active = false;

			var icon = iconByIndex.get(i);
			try { if (icon != null) icon.visible = icon.active = false; }
		...
		for (i in min...max)
		{
			...
			if (item != null)
			{
				...
				var scrollItem:Scrollable = cast(item, Scrollable);
				scrollItem.visible = scrollItem.active = true;
				scrollItem.x = ((scrollItem.targetY - lerpSelected) * scrollItem.distancePerItem.x) + scrollItem.startPosition.x;
				scrollItem.y = ((scrollItem.targetY - lerpSelected) * 1.3 * scrollItem.distancePerItem.y) + scrollItem.startPosition.y;

				var icon:HealthIcon = iconByIndex.get(i);
	...
	/**
	 * Called by APFreeplayManager when AP state changes (trap, unlock, etc.)
	 * Performs incremental reload instead of full state reload
	 */
	public function reloadDynamicSongs(?incrementalMode:Bool = true):Void
	{
		if (isDynamicLoading && fpManager != null && loadingQueue != null)
		{
			// Clear existing loaded items
			if (dynamicContainer != null)
				dynamicContainer.clear();
			if (iconContainer != null)
				iconContainer.clear();

			// Reinitialize for fresh load
			loadingTracker.initialize(fpManager.songList.length);
			loadingQueue = [];

			// Trigger reload with dynamic loading
			reloadSongs();
		}
		else
		{
			// Fallback to synchronous reload if not in dynamic mode
			reloadSongs();
		}
	}
