...
class MicdUpMenuState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		{
			spr.scale.set(FlxMath.lerp(spr.scale.x, 0.8, camLerp / (ClientPrefs.data.framerate / 60)), FlxMath.lerp(spr.scale.y, 0.8, 0.4 / (ClientPrefs.data.framerate / 60)));
			spr.y = FlxMath.lerp(spr.y, 40 + (spr.ID * 200), 0.4 / (ClientPrefs.data.framerate / 60));

			if (spr.ID == curSelected)
			{
				spr.scale.set(FlxMath.lerp(spr.scale.x, 1.1, camLerp / (ClientPrefs.data.framerate / 60)), FlxMath.lerp(spr.scale.y, 1.1, 0.4 / (ClientPrefs.data.framerate / 60)));
						...
						new FlxTimer().start(0.5, function(tmr:FlxTimer)
						{
							...
							switch (daChoice)
							{
								case 'story mode':
									...
									DiscordClient.changePresence("Going to the story mode selection.", null);
								case 'freeplay':
									FlxG.switchState(FreeplayState.new);
									DiscordClient.changePresence("Going to the freeplay selection.", null);
								case 'options':
