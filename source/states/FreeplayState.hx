...
class FreeplayState extends MusicBeatState
{
	...
	public static var searchBar:PsychUIInputText;
	...
	function changeDiff(change:Int = 0)
	{
		...
		try { curDifficulty = FlxMath.wrap(curDifficulty + change, 0, Difficulty.list.length-1);} catch(e) {}
		#if !switch
	...
	public function updateTexts(elapsed:Float = 0.0)
	{
		...
		for (i in min...max)
		{
			...
			try { item.visible = item.active = true; } catch(e) {}
