...
class JSChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		autosaveIndicator.antialiasing = ClientPrefs.data.globalAntialiasing;
	...
	function addSongUI():Void
	{
		...
		UI_songDiff = new FlxUIInputText(stageDropDown.x, stageDropDown.y + 40, 70, Difficulty.currentDifficulty, 8);
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.justPressed.ENTER)
			{
				...
				backend.StageData.loadDirectory(_song);
			...
			if (FlxG.keys.justPressed.BACKSPACE) {
				...
				openSubState(new Prompt('WARNING! This action will clear unsaved progress.\n\nProceed?', 0, 
					function() FlxG.switchState(states.editors.MasterEditorMenu.new), null,ignoreWarnings));
			}
		...
		curRenderedNotes.forEachAlive(function(note:Note) {
			...
			if(note.strumTime <= Conductor.songPosition) {
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					var noteDataToCheck:Int = note.noteData;
					if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += 4;
					if ((ClientPrefs.data.enableColorShader || ClientPrefs.data.showNotes && ClientPrefs.data.enableColorShader) && vortex)
	...
	function setupSusNote(note:Note, beats:Float):FlxSprite {
		...
		var color:FlxColor = (!PlayState.isPixelStage) ? ClientPrefs.data.arrowRGB[note.noteData][0] : ClientPrefs.data.arrowRGBPixel[note.noteData][0];
	...
	function loadJson(song:String, ?diff:String = ''):Void
	{
		...
		{
			if (diff != Difficulty.defaultDifficulty.toLowerCase()) {
				if(Difficulty.list[PlayState.storyDifficulty] == null || !diffJsonExists){
			...
			Difficulty.currentDifficulty = diff;
			FlxG.resetState();
			if (idleMusic != null && idleMusic.music != null) idleMusic.destroy();
		}
		else
