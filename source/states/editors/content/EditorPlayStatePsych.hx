...
class EditorPlayStatePsych extends MusicBeatSubstate
{
	...
	var _noteList:Array<ChartingNote>;
	...
	override function update(elapsed:Float)
	{
		...
		if (unspawnNotes[0] != null)
		{
			...
			{
				var dunceNote:ChartingNote = unspawnNotes[0];
		...
		if(notes.length > 0)
		{
			...
			notes.forEachAlive(function(daNote:ChartingNote)
			{
				...
				var strum:ChartingStrumNote = strumGroup.members[daNote.noteData];
				daNote.followChartingStrumNote(strum, fakeCrochet, songSpeed / playbackRate);
	...
	function generateSong()
	{
		...
		for (note in _noteList)
		{
			...
			var swagNote:ChartingNote = new ChartingNote(note.strumTime, note.noteData, oldNote, false, this);
			...
			if(roundSus > 0)
			{
				for (susNote in 0...roundSus)
				{
					...
					var sustainNote:ChartingNote = new ChartingNote(swagNote.strumTime + (curStepCrochet * susNote), note.noteData, oldNote, true, this);
					...
					if(!PlayState.isPixelStage)
					{
						...
						{
							oldNote.scale.y *= ChartingNote.SUSTAIN_SIZE / oldNote.frameHeight;
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...4)
		{
			...
			var babyArrow:ChartingStrumNote = new ChartingStrumNote(strumLineX, strumLineY, i, player);
	...
	{
		notes.forEachAlive(function(note:ChartingNote) invalidateNote(note));
	...
	private function popUpScore(note:ChartingNote = null):Void
	{
	...
	public static function sortHitNotes(a:ChartingNote, b:ChartingNote):Int
	{
	...
	private function keyPressed(key:Int)
	{
		...
		var plrInputNotes:Array<ChartingNote> = notes.members.filter(function(n:ChartingNote)
		...
		if (plrInputNotes.length != 0) { // slightly faster than doing `> 0` lol
			var funnyNote:ChartingNote = plrInputNotes[0]; // front note
			// trace('✡⚐🕆☼ 💣⚐💣');

			if (plrInputNotes.length > 1) {
				var doubleNote:ChartingNote = plrInputNotes[1];
		...
		var spr:ChartingStrumNote = playerStrums.members[key];
	...
	function goodNoteHit(note:ChartingNote):Void
	{
		...
		var spr:ChartingStrumNote = playerStrums.members[note.noteData];
	...
	public function invalidateNote(note:ChartingNote):Void {
		note.kill();
