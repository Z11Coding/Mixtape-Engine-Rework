...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	[
		...
		['Mult SV', "Value 1: Scroll Velocity Multiplier (1 is default)\nValue 2: Your usual tween options (EX: 5/sineInOut)"],
	...
	public static var GRID_COLUMNS_PER_PLAYER = Note.ammo[PlayfieldManager.mania[0]];
	...
	var strumLineNotes:FlxTypedGroup<ChartingStrumNote> = new FlxTypedGroup<ChartingStrumNote>();
	...
	var mixtapeMetadata:MetadataFile;
	...
	function refreshBalance():Void
	{
		...
		hurts1 = 0;
		hurts2 = 0;
		for (sec in PlayfieldManager.SONG.notes)
		{
	...
	var lilBf:FlxSprite;
	var lilBf2:FlxSprite;
	var lilOpp:FlxSprite;
	var lilOpp2:FlxSprite;
	var lilPlayer:Character;
	var lilPlayer2:Character;
	var lilOpponent:Character;
	var lilOpponent2:Character;
	var lilGf:Character;
	var gfSpeed:Int = 1;
	var singAnimations:Array<String> = ['singLEFT', 'singDOWN', 'singUP', 'singRIGHT'];

	public var reverseScroll:Bool = false;
	var reverseScrollEnabled:Bool = false;
	...
	override function create()
	{
		...
		idleMusic = new FlxSound();
		idleMusic.loadEmbedded(Paths.music(Paths.formatToSongPath('editorMusic/${ClientPrefs.data.editorMusic}')));
		idleMusic.volume = 0;
		idleMusic.pause();
		idleMusic.looped = true;
		idleMusic.autoDestroy = false;
		FlxG.sound.list.add(idleMusic);
		...
		lilBf2.visible = lilBf2.active = (lilBuddiesOn && PlayfieldManager.SONG.player4 != null);
		lilOpp2.visible = lilOpp2.active = (lilBuddiesOn && PlayfieldManager.SONG.player5 != null);
		...
		{
			var note:ChartingStrumNote = new ChartingStrumNote(startX + (GRID_SIZE * i), startY, i % GRID_COLUMNS_PER_PLAYER, i);
		...
		mainBox = new PsychUIBox(mainBoxPosition.x, mainBoxPosition.y, 300, 320, ['Charting', 'Data', 'Metadata', 'Events', 'Note', 'Section', 'Song']);
		...
		if(PlayfieldManager.SONG == null) //Atleast try to avoid crashes
		{
			openNewChart();
		}
	...
	function setReverseScroll(enabled:Bool) {
		...
		if (reverseScrollEditorButton != null) {
			reverseScrollEditorButton.text.text = reverseScrollEnabled ? '  Reverse Scroll ON' : '  Reverse Scroll OFF';
		}

		for(note in notes) {
			if(note == null) continue;
			note.reverseScroll = reverseScroll;
		...
		for(event in events) {
			...
			var secNum:Int = 0;
			for (i in 1...cachedSectionTimes.length) {
				if(cachedSectionTimes[i] > event.strumTime) break;
				secNum++;
			}
		...
		if(waveformEnabled) {
			waveformSprite.flipY = reverseScroll;
			if(reverseScroll) {
				waveformSprite.y = gridBg.y + gridBg.height - waveformSprite.height;
			} else {
	...
	function createLilPlayer2(name:String = '')
	{
		//customFunctions
		//shows actual characters (From Moon's Modded Psych Engine)
		if(name != null && name != "" && name != " " && name != "null") {
			lilPlayer2 = new Character(0, 0, name, false);
			lilPlayer2.visible = false;
			lilPlayer2.scrollFactor.set();
			lilPlayer2.screenCenter();
			lilPlayer2.x += lilPlayer2DP[0];
			lilPlayer2.y += lilPlayer2DP[1];
			lilPlayer2.setGraphicSize(Std.int(lilPlayer2.width * 0.4));
			add(lilPlayer2);
			lilPlayer2.flipX = !lilPlayer2.flipX;
			for (key in lilPlayer2.animOffsets.keys()) {
				lilPlayer2.animOffsets[key][0] *= lilPlayer2.scale.x;
				lilPlayer2.animOffsets[key][1] *= lilPlayer2.scale.y;
			}
			lilPlayer2.dance();
		} else {
			lilPlayer2 = new Character(0, 0, 'bf', false);
			lilPlayer2.visible = false;
		}
	}

	function createLilOpponent(name:String = 'dad')
	{
		if(name != null && name != '') {
	...
	function createLilOpponent2(name:String = '')
	{
		if(name != null && name != '') {
			lilOpponent2 = new Character(0, 0, name, false);
			lilOpponent2.scrollFactor.set();
			lilOpponent2.screenCenter();
			lilOpponent2.x += lilOpponentDP[0];
			lilOpponent2.y += lilOpponentDP[1];
			lilOpponent2.setGraphicSize(Std.int(lilOpponent2.width * 0.4));
			add(lilOpponent2);
			for (keyt in lilOpponent2.animOffsets.keys()) {
				lilOpponent2.animOffsets[keyt][0] *= lilOpponent2.scale.x;
				lilOpponent2.animOffsets[keyt][1] *= lilOpponent2.scale.y;
			}
			lilOpponent2.dance();
		} else {
			lilOpponent2 = new Character(0, 0, '', false);
			lilOpponent2.visible = false;
		}
	}
	...
	function reloadLilBuddies(id:Int = 6) //id 1 is for the player, id 2 is for the opponent, id 3 is for Girlfriend (or middle), id 5 is for the player2, id 6 is for all
	{
		var character1 = PlayfieldManager.SONG.player1;
		var character2 = PlayfieldManager.SONG.player2;
		var character3 = PlayfieldManager.SONG.gfVersion;
		var character4 = PlayfieldManager.SONG.player4;
		var character5 = PlayfieldManager.SONG.player5;

		if(id == 1 || id == 6)//Reload The Player
			remove(lilPlayer);
			createLilPlayer(character1);
		if(id == 2 || id == 6)//Reload The Opponent
			remove(lilOpponent);
			createLilOpponent(character2);
		if(id == 3 || id == 6) //Reload The Girlfriend
			remove(lilGf);
			createLilGirlfriend(character3);
		if(PlayfieldManager.SONG.player4 != null && (id == 4 || id == 6))//Reload The Other Opponent
			remove(lilOpponent2);
			createLilOpponent2(character4);
		if(PlayfieldManager.SONG.player5 != null && (id == 5 || id == 6))//Reload The Other Player
			remove(lilPlayer2);
	...
	{
		if(PlayfieldManager.SONG == null) return;

		// SONG TAB
		songNameInputText.text = PlayfieldManager.SONG.song;
		allowVocalsCheckBox.checked = (PlayfieldManager.SONG.needsVoices != false); //If the song for some reason does not have this value, it will be set to true

		maniaStepper.value = PlayfieldManager.SONG.mania;
		startManiaStepper.value = PlayfieldManager.SONG.startMania;
		GRID_COLUMNS_PER_PLAYER = Note.ammo[PlayfieldManager.SONG.mania];
		GRID_SIZE = Note.gridSizes[PlayfieldManager.SONG.mania];
		changeMania();

		bpmStepper.value = PlayfieldManager.SONG.bpm;
		scrollSpeedStepper.value = PlayfieldManager.SONG.speed;
		audioOffsetStepper.value = Reflect.hasField(PlayfieldManager.SONG, 'offset') ? PlayfieldManager.SONG.offset : 0;
		Conductor.offset = audioOffsetStepper.value;

		playerDropDown.selectedLabel = PlayfieldManager.SONG.player1;
		player2DropDown.selectedLabel = PlayfieldManager.SONG.player5;
		opponentDropDown.selectedLabel = PlayfieldManager.SONG.player2;
		opponent2DropDown.selectedLabel = PlayfieldManager.SONG.player4;
		girlfriendDropDown.selectedLabel = PlayfieldManager.SONG.gfVersion;
		stageDropDown.selectedLabel = PlayfieldManager.SONG.stage;
		StageData.loadDirectory(PlayfieldManager.SONG);

		// DATA TAB
		gameOverCharDropDown.selectedLabel = PlayfieldManager.SONG.gameOverChar;
		gameOverSndInputText.text = PlayfieldManager.SONG.gameOverSound;
		gameOverLoopInputText.text = PlayfieldManager.SONG.gameOverLoop;
		gameOverRetryInputText.text = PlayfieldManager.SONG.gameOverEnd;

		noRGBCheckBox.checked = (PlayfieldManager.SONG.disableNoteRGB == true);

		noteTextureInputText.text = PlayfieldManager.SONG.arrowSkin;
		noteSplashesInputText.text = PlayfieldManager.SONG.splashSkin;
	...
	private var player2HoldTime:Float = 0;
		...
		*/
		...
		if(autoSaveCap > 0)
		{
			...
			if(autoSaveTime >= autoSaveCap #if debug || FlxG.keys.justPressed.NUMPADMULTIPLY #end)
			{
				...
				var songCopy:SwagSong = Reflect.copy(PlayfieldManager.SONG);
		...
		if(FlxG.sound.music != null)
		{
			...
			if(charterFocus) //If not typing anything
			{
				...
				if(FlxG.keys.justPressed.A != FlxG.keys.justPressed.D && !holdingAlt)
				{
					...
					{
						if(curSec + shiftAdd >= PlayfieldManager.SONG.notes.length) shiftAdd = PlayfieldManager.SONG.notes.length - curSec - 1;
				...
				else if(FlxG.keys.justPressed.END)
				{
					...
					loadSection(PlayfieldManager.SONG.notes.length - 1);
				}
		...
		if(charterFocus)
		{
			...
			if(FlxG.keys.justPressed.ENTER #if ARCHIPELAGO_ALLOWED || archipelago.APEntryState.inArchipelagoMode #end)
			{
				goToPlayState();
				return;
			}
				...
				FlxG.keys.justPressed.C || FlxG.keys.justPressed.V || FlxG.keys.justPressed.A || FlxG.keys.justPressed.S))
			{
				...
				else if(FlxG.keys.justPressed.V) // Paste (Ctrl + V)
				{
					if(copiedNotes.length > 0 || copiedEvents.length > 0)
					{
						...
						selectedNotes.sort(PlayfieldManager.sortByTime);
			...
			else if(canContinue)
			{
				...
				else if(FlxG.keys.justPressed.Z != FlxG.keys.justPressed.X) //Decrease/Increase Zoom
				{
					...
					notes.sort(PlayfieldManager.sortByTime);
					var noteSec:Int = 0;
				...
				else if(FlxG.mouse.x >= gridBg.x && FlxG.mouse.x < gridBg.x + gridBg.width)
				{
					...
					else if(!holdingAlt && FlxG.mouse.y >= gridBg.y && FlxG.mouse.y < gridBg.y + gridBg.height) // Add note
					{
						...
						if(reverseScroll) {
							...
							var sectionHeight = gridBg.height;

							// basically we invert the shits obviously
							var pct = 1.0 - ((FlxG.mouse.y - gridBg.y) / sectionHeight);

							// apply this when holding Shift (currently goes above mouse pointer but why do we need this anyway?)
							if(!FlxG.keys.pressed.SHIFT) {
								var snapSize = GRID_SIZE / (curQuant/16);
								var snappedY = Math.floor((sectionHeight * pct) / snapSize) * snapSize;
								pct = snappedY / sectionHeight;
							}

							strumTime = sectionStart + (pct * (sectionEnd - sectionStart)); // places note on reverseScroll
						} else {
							strumTime = (diffY / GRID_SIZE * Conductor.stepCrochet / curZoom) + cachedSectionTimes[curSec]; // original strumTime float used on upScroll
						}
		...
		if(Conductor.songPosition != lastTime || forceDataUpdate)
		{
			...
			var str:String =  '$curTime / $songLength' +
							  ...
							  '\nSelected: ${selectedNotes.length}' +
								'\n\nDupes in this section: ${countDupes(PlayfieldManager.SONG.notes[curSec])}' +
								'\nDupes total: $dupes' +
			...
			for (note in selectedNotes)
			{
				...
				if(!note.isEvent)
				{
					...
					if(charterFocus && qPress != ePress)
					{
						...
						note.setSustainLength(note.sustainLength + addSus, cachedSectionCrochets[noteSec] / 4, curZoom, reverseScroll);
	...
	function callBeatHit(curBeat)
	{
		...
		if (PlayfieldManager.SONG.player5 != null && curBeat % lilPlayer2.danceEveryNumBeats == 0 && !lilPlayer2.getAnimationName().startsWith('sing')) {
			lilPlayer2.dance();
		}
		...
		if (PlayfieldManager.SONG.player4 != null && lilOpponent2 != null && curBeat % lilOpponent2.danceEveryNumBeats == 0 && !lilOpponent2.getAnimationName().startsWith('sing')) {
			lilOpponent2.dance();
		}
	...
	function hitNote(note:MetaNote) {
		...
		var data:Int = note.noteData % Note.ammo[PlayfieldManager.mania[0]]; // this isn't used but keep it just incase (lilBuddies)
		...
		var player2ShouldSing:Bool = false;
		var opponentShouldSing:Bool = false;
		var opponent2ShouldSing:Bool = false;
		...
		if(note.gfNote) { // literally if it's GF notes use them
			...
			if(isGfSide) { // simple enough
				...
				if(sec.mustHitSection) {
					opponentShouldSing = (!note.mustPress && !note.exNote);
					opponent2ShouldSing = (!note.mustPress && note.exNote);
				} else {
					player2ShouldSing = (note.mustPress && note.exNote);
					playerShouldSing = (note.mustPress && !note.exNote);
				}
			}
		}
		else {
			...
			if(note.mustPress) {
				if (note.exNote)
					player2ShouldSing = true;
				else
					playerShouldSing = true;
			} else {
				if (note.exNote)
					opponent2ShouldSing = true;
				else
		...
		if(playerShouldSing) {
			lilPlayer.playAnim(singAnimations[note.noteData] + note.animSuffix, true);
		...
		if(player2ShouldSing) {
			lilPlayer2.playAnim(singAnimations[note.noteData] + note.animSuffix, true);
			lilPlayer2.holdTimer = -Math.max(Conductor.stepCrochet * 1.25, note.sustainLength) / 1000 / playbackRate;
			player2HoldTime = ((Conductor.stepCrochet * 1.25) + note.sustainLength) / 1000 / playbackRate; // now it holds on long notes yay
		}

		if(opponentShouldSing) {
			lilOpponent.playAnim(singAnimations[note.noteData] + note.animSuffix, true);
		...
		if(opponent2ShouldSing) {
			lilOpponent2.playAnim(singAnimations[note.noteData] + note.animSuffix, true);
			lilOpponent2.holdTimer = -Math.max(Conductor.stepCrochet * 1.25, note.sustainLength) / 1000 / playbackRate;
			opponentHoldTime = ((Conductor.stepCrochet * 1.25) + note.sustainLength) / 1000 / playbackRate;
		}

		if(gfShouldSing) {
			lilGf.playAnim(singAnimations[note.noteData] + note.animSuffix, true);
		...
		if(lilBuddiesSing)
		{
			data += Note.ammo[PlayfieldManager.mania[0]];
			if (!note.noAnimation) {
				if (note.mustPress)
				{
					if (note.exNote) {
						lilBf2.animation.play("" + (note.noteData % Note.ammo[PlayfieldManager.mania[0]]), true);
						lilBf2.color = note.rgbShader.r;
						lilBf2ResetAnim = ((Conductor.stepCrochet * PlayfieldManager.mania[0]) + note.sustainLength) / 1000 / playbackRate; // for lil buddies to reset after hitting notes. It doesn't stop at sections anymore.
					} else {
						lilBf.animation.play("" + (note.noteData % Note.ammo[PlayfieldManager.mania[0]]), true);
						lilBf.color = note.rgbShader.r;
						lilBfResetAnim = ((Conductor.stepCrochet * PlayfieldManager.mania[0]) + note.sustainLength) / 1000 / playbackRate; // for lil buddies to reset after hitting notes. It doesn't stop at sections anymore.
					}
				}
				else
				{
					if (note.exNote) {
						lilOpp2.animation.play("" + (note.noteData % Note.ammo[PlayfieldManager.mania[0]]), true);
						lilOpp2.color = note.rgbShader.r;
						lilOpp2ResetAnim = ((Conductor.stepCrochet * PlayfieldManager.mania[0]) + note.sustainLength) / 1000 / playbackRate;
					} else {
						lilOpp.animation.play("" + (note.noteData % Note.ammo[PlayfieldManager.mania[0]]), true);
						lilOpp.color = note.rgbShader.r;
						lilOppResetAnim = ((Conductor.stepCrochet * PlayfieldManager.mania[0]) + note.sustainLength) / 1000 / playbackRate;
					}
				}
			}
		}
	...
	function stopMovingNotes() //This turns moving notes into saved notes
	{
		...
		notes.sort(PlayfieldManager.sortByTime);
		events.sort(PlayfieldManager.sortByTime);
		movingNotes.clear();
		isMovingNotes = false;
		softReloadNotes();
	}
	...
	function updateScrollY()
	{
		...
		var sectionY:Float = secRows * GRID_SIZE * curZoom;
		var timeOffset:Float = ((Conductor.songPosition - secStartTime) / secCrochet * GRID_SIZE * 4) * curZoom;

		if(reverseScroll)
		{
			// stays constant with the scrolling
			var sectionHeight:Float = PlayfieldManager.SONG.notes[curSec].sectionSteps * PlayfieldManager.SONG.notes[curSec].sectionBeats * GRID_SIZE * curZoom;
			scrollY = sectionY + sectionHeight - timeOffset - GRID_SIZE - FlxG.height/2;
		}
		else
		{
			// ofcourse use original code for UpScroll
			scrollY = sectionY + timeOffset - FlxG.height/2;
		}
	}
	...
	function updateSelectionBox()
	{
		...
		selectionBox.scale.set(diffX/Note.ammo[PlayfieldManager.SONG.mania], diffY/Note.ammo[PlayfieldManager.SONG.mania]);
	...
	function loadMetadata() {
		try {mixtapeMetadata = cast Json.parse(File.getContent(Paths.json(Paths.formatToSongPath(PlayfieldManager.SONG.song.toLowerCase()) + '/meta')));}
		catch(e) {
	...
	function loadMusic(?killAudio:Bool = false)
	{
		...
		if(killAudio)
		{
			...
			{
				//trace(key, snd);
				if(key.contains('/songs/${Paths.formatToSongPath(PlayfieldManager.SONG.song)}/') && snd != null)
				{
					sndsToKill.push(key);
					snd.close();
				}
		...
		try
		{
			FlxG.sound.playMusic(Paths.inst(PlayfieldManager.SONG.song), 0);
		...
		if (PlayfieldManager.SONG.needsVoices)
		{
			try
			{
				var playerVocals:Sound = Paths.voices(PlayfieldManager.SONG.song, (characterData.vocalsP1 == null || characterData.vocalsP1.length < 1) ? 'Player' : characterData.vocalsP1);
				vocals.loadEmbedded(playerVocals != null ? playerVocals : Paths.voices(PlayfieldManager.SONG.song));
				vocals.volume = 0;
				vocals.play();
				vocals.pause();
				vocals.time = time;

				var oppVocals:Sound = Paths.voices(PlayfieldManager.SONG.song, (characterData.vocalsP2 == null || characterData.vocalsP2.length < 1) ? 'Opponent' : characterData.vocalsP2);
		...
		#if DISCORD_ALLOWED
		DiscordClient.changePresence('Chart Editor', 'Song: ' + PlayfieldManager.SONG.song);
		#end
	...
	function onSongComplete()
	{
		...
		curSec = PlayfieldManager.SONG.notes.length - 1;
		forceDataUpdate = true;
	}
	...
	function setSongPlaying(doPlay:Bool)
	{
		...
		else
		{
			...
			vocals.pause();
			opponentVocals.pause();
			idleTimer.start(10, function(tmr:FlxTimer) {
				idleMusic.resume();
				idleMusic.fadeIn(5, 0, ClientPrefs.data.editorMusVol);
			});
		}
	...
	function reloadNotes()
	{
		...
		for (eventNum => event in PlayfieldManager.SONG.events)
	...
	function createNote(note:Dynamic, ?secNum:Null<Int> = null)
	{
		...
		var section = PlayfieldManager.SONG.notes[secNum];
		...
		try {
			...
			swagNote.changeNoteData(daNoteData);
			swagNote.reverseScroll = reverseScroll; // Set reverse scroll state
			swagNote.mustPress = gottaHitNote;
			swagNote.setSustainLength(note[2], cachedSectionCrochets[secNum] / 4, curZoom, reverseScroll);
	...
	function _cacheSections()
	{
		...
		var bpm:Float = PlayfieldManager.SONG.bpm;
		...
		for (secNum => section in PlayfieldManager.SONG.notes)
		{
			...
			for (note in section.sectionNotes)
			{
				...
				else if(secNum < PlayfieldManager.SONG.notes.length && note[0] >= time - 0.000001) note[0] = time - 0.000001;
			}

			if(FlxG.sound.music != null && time >= FlxG.sound.music.length)
			{
				var lastSectionNum:Int = PlayfieldManager.SONG.notes.length - 1;
				if(secNum < lastSectionNum) //Delete extra sections
				{
					while(PlayfieldManager.SONG.notes.length - 1 > secNum)
					{
						PlayfieldManager.SONG.notes.pop();
					}
		...
		{
			var lastSection = PlayfieldManager.SONG.notes[PlayfieldManager.SONG.notes.length-1];
			var beat:Float = Conductor.calculateCrochet(bpm);
			...
			while(!reachedLimit)
			{
				PlayfieldManager.SONG.notes.push({
					sectionNotes: [],
				...
				{
					trace('created sections until ${PlayfieldManager.SONG.notes.length-1}');
					reachedLimit = true;
				}
			}
		}
	...
	function loadSection(?sec:Null<Int> = null)
	{
		...
		curSec = Std.int(FlxMath.bound(curSec, 0, PlayfieldManager.SONG.notes.length-1));
		Conductor.bpm = cachedSectionBPMs[curSec];

		var hei:Float = 0;

		// THIS IS TO ORDER THE SECTIONS!!
		if(reverseScroll)
		{
			if(nextGridBg.exists) remove(nextGridBg);
			if(gridBg.exists) remove(gridBg);
			if(prevGridBg.exists) remove(prevGridBg);
		...
		{
			if(reverseScroll) {
				prevGridBg.y = gridBg.y + gridBg.height;
			} else {
				prevGridBg.y = cachedSectionRow[curSec-1] * GRID_SIZE * curZoom;
			}
			prevGridBg.rows = PlayfieldManager.SONG.notes[curSec-1].sectionSteps * PlayfieldManager.SONG.notes[curSec-1].sectionBeats * curZoom;
		...
		if(curSec < PlayfieldManager.SONG.notes.length - 1)
		{
			if(reverseScroll) {
				nextGridBg.y = gridBg.y - nextGridBg.height;
			} else {
				nextGridBg.y = cachedSectionRow[curSec+1] * GRID_SIZE * curZoom;
			}
			nextGridBg.rows = PlayfieldManager.SONG.notes[curSec+1].sectionSteps * PlayfieldManager.SONG.notes[curSec+1].sectionBeats * curZoom;
		...
		gridBg.rows = PlayfieldManager.SONG.notes[curSec].sectionSteps * PlayfieldManager.SONG.notes[curSec].sectionBeats * curZoom;
		hei += gridBg.height;
	...
	function softReloadNotes(onlyCurrent:Bool = false)
	{
		...
		function prevSecFilter(note:MetaNote)
		{
			return (note.strumTime >= prevMinTime && note.strumTime < prevMaxTime);
		}

		var nextMinTime:Float = getMinNoteTime(curSec+1);
		var nextMaxTime:Float = getMaxNoteTime(curSec+1);
		function nextSecFilter(note:MetaNote)
		{
			return (note.strumTime >= nextMinTime && note.strumTime < nextMaxTime);
		}
		...
		if(SHOW_EVENT_COLUMN)
		{
			for (num => event in events)
			{
				if(event != null && curSecFilter(event))
				{
					...
					positionNoteYOnTime(event, curSec);
				}
			}
		}
		...
		if(!onlyCurrent)
		{
			if(curSec > 0 && prevGridBg.visible)
			{
				for(note in notes)
				{
					if(note != null && prevSecFilter(note))
					{
						behindRenderedNotes.add(note);
						note.alpha = 1;
						note.reverseScroll = reverseScroll;

						// Complicated shit just to show notes correctly (previous sections)
						if(reverseScroll) {
							var prevSecY = gridBg.y + gridBg.height;
							var relativeY = (note.strumTime - prevMinTime) / (prevMaxTime - prevMinTime) * prevGridBg.height;

							if(reverseScroll) {
								relativeY = prevGridBg.height - relativeY - GRID_SIZE;
							}

							note.y = prevSecY + relativeY + (GRID_SIZE/2 - note.height/2);
							note.chartY = prevSecY + relativeY;
						} else {
				...
				if(SHOW_EVENT_COLUMN)
				{
					for(event in events)
					{
						if(event != null && prevSecFilter(event))
						{
							...
							if(reverseScroll) {
								var prevSecY = gridBg.y + gridBg.height;
								var relativeY = (event.strumTime - prevMinTime) / (prevMaxTime - prevMinTime) * prevGridBg.height;

								if(reverseScroll) {
									relativeY = prevGridBg.height - relativeY - GRID_SIZE;
								}

								event.y = prevSecY + relativeY + (GRID_SIZE/2 - event.height/2);
								event.chartY = prevSecY + relativeY;
							} else {
								positionNoteYOnTime(event, curSec-1);
							}
						}
					}
				}
			...
			if(curSec < PlayfieldManager.SONG.notes.length - 1 && nextGridBg.visible)
			{
				for(note in notes)
				{
					if(note != null && nextSecFilter(note))
					{
						behindRenderedNotes.add(note);
						note.alpha = 1;
						note.reverseScroll = reverseScroll;

						// Now we do the complicated shit for next sections
						if(reverseScroll) {
							var nextSecY = gridBg.y - nextGridBg.height;
							var relativeY = (note.strumTime - nextMinTime) / (nextMaxTime - nextMinTime) * nextGridBg.height;

							if(reverseScroll) {
								relativeY = nextGridBg.height - relativeY - GRID_SIZE;
							}

							note.y = nextSecY + relativeY + (GRID_SIZE/2 - note.height/2);
							note.chartY = nextSecY + relativeY;
						} else {
							positionNoteYOnTime(note, curSec+1);
						}
				...
				if(SHOW_EVENT_COLUMN)
				{
					for(event in events)
					{
						if(event != null && nextSecFilter(event))
						{
							behindRenderedNotes.add(event);
							event.alpha = 1;
							event.eventText.visible = false;

							// Same for events (give me a break man)
							if(reverseScroll) {
								var nextSecY = gridBg.y - nextGridBg.height;
								var relativeY = (event.strumTime - nextMinTime) / (nextMaxTime - nextMinTime) * nextGridBg.height;

								if(reverseScroll) {
									relativeY = nextGridBg.height - relativeY - GRID_SIZE;
								}

								event.y = nextSecY + relativeY + (GRID_SIZE/2 - event.height/2);
								event.chartY = nextSecY + relativeY;
							} else {
								positionNoteYOnTime(event, curSec+1);
							}
						}
	...
	function positionNoteYOnTime(note:MetaNote, section:Int)
	{
		...
		var sectionY:Float = cachedSectionRow[section] * GRID_SIZE * curZoom;
		var timeOffset:Float = (time / cachedSectionCrochets[section]) * GRID_SIZE * 4 * curZoom; // Renamed because i wanted to look cool

		var noteY:Float;
		// Never touching this ever again
		if(reverseScroll) {
			var sectionHeight:Float = PlayfieldManager.SONG.notes[section].sectionSteps * PlayfieldManager.SONG.notes[section].sectionBeats * GRID_SIZE * curZoom;
	...
	{
		var curSecData:SwagSection = PlayfieldManager.SONG.notes[curSec];
	...
	function addDataTab()
	{
		...
		{
			PlayfieldManager.SONG.gameOverChar = character;
			if(character.length < 1) Reflect.deleteField(PlayfieldManager.SONG, 'gameOverChar');
		...
		{
			PlayfieldManager.SONG.gameOverSound = cur;
			if(cur.trim().length < 1) Reflect.deleteField(PlayfieldManager.SONG, 'gameOverSound');
		}
		objY += 40;
		gameOverLoopInputText = new PsychUIInputText(objX, objY, 120, '', 8);
		gameOverLoopInputText.onChange = function(old:String, cur:String)
		{
			PlayfieldManager.SONG.gameOverLoop = cur;
			if(cur.trim().length < 1) Reflect.deleteField(PlayfieldManager.SONG, 'gameOverLoop');
		}
		objY += 40;
		...
		{
			PlayfieldManager.SONG.gameOverEnd = cur;
			if(cur.trim().length < 1) Reflect.deleteField(PlayfieldManager.SONG, 'gameOverEnd');
		}
		...
		noteTextureInputText.unfocus = function()
		{
			...
			if(PlayfieldManager.SONG.arrowSkin != noteTextureInputText.text) changed = true;
			PlayfieldManager.SONG.arrowSkin = noteTextureInputText.text.trim();
			if(PlayfieldManager.SONG.arrowSkin.trim().length < 1) PlayfieldManager.SONG.arrowSkin = null;
		...
		{
			PlayfieldManager.SONG.splashSkin = cur;
			if(cur.trim().length < 1) PlayfieldManager.SONG.splashSkin = null;
		}
	...
	function addMetadataTab()
	{
		...
		{
			mixtapeMetadata.freeplay.bg = cur;
		}
		...
		{
			mixtapeMetadata.freeplay.album = cur;
		}
		...
		{
			mixtapeMetadata.freeplay.ratings.set(Difficulty.list[PlayState.storyDifficulty].toLowerCase(), Std.int(ratingsStepper.value));
		}
		...
		susLengthStepper.onValueChange = function()
		{
			...
			if(susLengthLastVal != susLengthStepper.value)
			{
				if(selectedNotes.length > 1)
				{
					...
					{
						if(note == null && !note.isEvent) continue;
						note.setSustainLength(note.sustainLength + (susLengthStepper.value - susLengthLastVal), Conductor.stepCrochet, curZoom, reverseScroll);
					}
				}
				else if(selectedNotes.length == 1) selectedNotes[0].setSustainLength(susLengthStepper.value, Conductor.stepCrochet, curZoom, reverseScroll);
	...
	function addSongTab()
	{
		...
		songNameInputText.onChange = function(old:String, cur:String) PlayfieldManager.SONG.song = cur;

		allowVocalsCheckBox = new PsychUICheckBox(objX, objY + 20, 'Allow Vocals', 80, function()
		{
			PlayfieldManager.SONG.needsVoices = allowVocalsCheckBox.checked;
			loadMusic();
		});
		...
		{
			var oldTimes:Array<Float> = cachedSectionTimes.copy();
			PlayfieldManager.SONG.bpm = bpmStepper.value;
			adaptNotesToNewTimes(oldTimes);
		};

		scrollSpeedStepper = new PsychUINumericStepper(objX + 90, objY, 0.1, 1, 0.1, 10, 2);
		scrollSpeedStepper.onValueChange = function() PlayfieldManager.SONG.speed = scrollSpeedStepper.value;

		audioOffsetStepper = new PsychUINumericStepper(objX + 180, objY, 1, 0, -500, 500, 0);
		audioOffsetStepper.onValueChange = function()
		{
			PlayfieldManager.SONG.offset = audioOffsetStepper.value;
			Conductor.offset = audioOffsetStepper.value;
			updateWaveform();
		};
		...
		{
			PlayfieldManager.SONG.player1 = character;
			remove(lilPlayer);
			createLilPlayer(character);
			updateJsonData();
			updateHeads(true);
			loadMusic();
			//switchCharacters(character, false); //TODO: Get this working properly
			trace('selected $character');
		});

		opponentDropDown = new PsychUIDropDownMenu(objX, objY + 40, [''], function(id:Int, character:String)
		{
			PlayfieldManager.SONG.player2 = character;
			remove(lilOpponent);
			createLilOpponent(character);
			updateJsonData();
			updateHeads(true);
			loadMusic();
			//switchCharacters(character, true); //TODO: Get this working properly
			trace('selected $character');
		});

		opponent2DropDown = new PsychUIDropDownMenu(objX + 140, objY + 40, [''], function(id:Int, character:String)
		{
			PlayfieldManager.SONG.player4 = character;
			remove(lilOpponent2);
			createLilOpponent2(character);
			updateJsonData();
			updateHeads(true);
			loadMusic();
			//switchCharacters(character, true); //TODO: Get this working properly
		...
		player2DropDown = new PsychUIDropDownMenu(objX + 140, objY, [''], function(id:Int, character:String)
		{
			if (character != null && character != "") {
				PlayfieldManager.SONG.player5 = character;
				remove(lilPlayer2);
				createLilPlayer2(character);
				updateJsonData();
				updateHeads(true);
				loadMusic();
				//switchCharacters(character, false); //TODO: Get this working properly
		...
		{
			PlayfieldManager.SONG.gfVersion = character;
			remove(lilGf);
		...
		{
			PlayfieldManager.SONG.stage = stage;
			StageData.loadDirectory(PlayfieldManager.SONG);
			trace('selected $stage');
		});

		maniaStepper = new PsychUINumericStepper(objX + 140, objY + 120, 1, 3, 0, 17, 0);
		maniaStepper.onValueChange = function()
		{
			PlayfieldManager.SONG.mania = Std.int(maniaStepper.value);
			PlayfieldManager.mania[0] = Std.int(maniaStepper.value);
			GRID_COLUMNS_PER_PLAYER = Note.ammo[PlayfieldManager.mania[0]];
			GRID_SIZE = Note.gridSizes[PlayfieldManager.mania[0]];
			changeMania();
		};

		startManiaStepper = new PsychUINumericStepper(objX, objY + 120, 1, 3, 0, 17, 0);
		startManiaStepper.onValueChange = function()
		{
			PlayfieldManager.SONG.startMania = Std.int(startManiaStepper.value);
		};
	...
	function addFileTab()
	{
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Save Metadata as...', function()
		{
			...
			if (ImprovedFileHandling.saveOperation('Save Metadata (meta.json)', {ext: "json", desc: "JSON File"}, Text, PsychJsonPrinter.print(mixtapeMetadata)))
		...
		if(SHOW_EVENT_COLUMN)
		{
			...
			var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Save Events...', function()
			{
				...
				if (ImprovedFileHandling.saveOperation('Save Events (events.json)', {ext: "json", desc: "JSON File"}, Text, PsychJsonPrinter.print({events: PlayfieldManager.SONG.events, format: 'psych_v1'}, ['events'])))
					showOutput('Events saved successfully!');
				else
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Save (V-Slice)...', function()
		{
			...
			function doJsonStuff(fileDialog:String):Dynamic
			{
				try
				{
					var path:String = fileDialog.replace('\\', '/');

					var chartName:String = Paths.formatToSongPath(PlayfieldManager.SONG.song) + '.json';
					chartName = chartName.substring(chartName.lastIndexOf('/')+1, chartName.lastIndexOf('.'));
					...
					updateChartData();
					var pack:VSlicePackage = VSlice.export(PlayfieldManager.SONG);
			...
			doJsonStuff(ImprovedFileHandling.selectFolder('Save V-Slice Chart/Metadata JSONs'));
		},btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Psych to V-Slice...', function()
		{
						...
						var btn:PsychUIButton = new PsychUIButton(btnX, btnY, 'Save', function()
						{
							try
							{
								...
								if(diffs != null && diffs.length > 0)
								{
									...
									function doFolderStuff(fileDialog:String)
									{
										overwriteSavedSomething = false;
										var path:String = fileDialog.replace('\\', '/');
									...
									doFolderStuff(ImprovedFileHandling.selectFolder('Save V-Slice Chart/Metadata JSONs'));
								}
			...
			doJsonStuff(ImprovedFileHandling.openFile('Open a Psych Engine Chart JSON', [{ext: "json", desc: "JSON File"}]));
		},btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  V-Slice to Psych...', function()
		{
			...
			function doJsonStuff(fileDialog:String)
			{
				...
				var chart:VSliceChart = cast Json.parse(File.getContent(fileDialog));
				...
				function doJsonStuffPart2(fileDialog:String)
				{
					var metadata:VSliceMetadata = cast Json.parse(File.getContent(fileDialog));
					...
					try
					{
						...
						{
							function doJsonStuffPart3(fileDialog:String)
							{
								var path:String = fileDialog.replace('\\', '/');
								...
								function nextChart()
								{
									...
									if(pack.events != null)
									{
										...
										{
											if(overwriteSavedSomething)
												showOutput('Files saved successfully to: ${fileDialog}!');
										}, true);
									}
				...
				doJsonStuffPart2(ImprovedFileHandling.openFile('Open a V-Slice Metadata file', [{ext: "json", desc: "JSON File"}]));
			}
			doJsonStuff(ImprovedFileHandling.openFile('Open a V-Slice Chart file', [{ext: "json", desc: "JSON File"}]));
		},btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Update (Legacy)...', function()
		{
			...
			function doJsonStuff(fileDialog:String)
			{
				var oldSong = PlayfieldManager.SONG;
				try
				{
					var filePath:String = fileDialog.replace('\\', '/');
					var jsonFile:Dynamic = Json.parse(File.getContent(filePath));
					...
					var loadedChart:SwagSong = Song.parseJSON(jsonFile, filePath, '');
					if(loadedChart == null || !Reflect.hasField(loadedChart, 'song')) //Check if chart is ACTUALLY a chart and valid
					{
						showOutput('Error: File loaded is not a Psych Engine 0.x.x/FNF 0.2.x.x chart.', true);
						return;
					...
					if(!fmt.startsWith('psych_v1'))
					{
						...
						File.saveContent(filePath, PsychJsonPrinter.print(loadedChart, ['sectionNotes', 'events']));
	...
	function addEditTab()
	{
		...
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Clear All Note Dupes', function()
		{
			removeDupes();
		}, btnWid);
		btn.text.alignment = LEFT;
	...
	function addViewTab()
	{
		...
		btnY++;
		btnY += 20;
		reverseScrollEditorButton = new PsychUIButton(btnX, btnY, reverseScrollEnabled ? '  Reverse Scroll ON' : '  Reverse Scroll OFF', function()
		{
			// The ultimate button!!
			reverseScrollEnabled = !reverseScrollEnabled;
			chartEditorSave.data.reverseScroll = reverseScrollEnabled;
		    setReverseScroll(reverseScrollEnabled);
		}, btnWid);
		reverseScrollEditorButton.text.alignment = LEFT;
		tab_group.add(reverseScrollEditorButton);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Go to...', function()
		{
			...
			openSubState(new BasePrompt(420, 200, 'Go to Time/Section:',
				function(state:BasePrompt)
				{
					...
					timeStepper.cameras = state.cameras;
					var sectionStepper:PsychUINumericStepper = new PsychUINumericStepper(timeStepper.x + 160, timeStepper.y, 1, currentSec, 0, PlayfieldManager.SONG.notes.length - 1, 0);
	...
	{
		for (secNum => section in PlayfieldManager.SONG.notes)
			PlayfieldManager.SONG.notes[secNum].sectionNotes = [];
		...
		for (num => note in notes)
		{
			...
			var arr:Array<Dynamic> = PlayfieldManager.SONG.notes[noteSec].sectionNotes;
			//trace('Added note with time ${note.songData[0]} at section $noteSec');
			arr.push(note.songData);
		}

		events.sort(PlayfieldManager.sortByTime);
		PlayfieldManager.SONG.events = [];
		for (event in events)
			PlayfieldManager.SONG.events.push(event.songData);
	}

	function saveChart(canQuickSave:Bool = true)
	{
		updateChartData();
		var chartData:String = PsychJsonPrinter.print(PlayfieldManager.SONG, ['sectionNotes', 'events']);
		...
		else
		{
			...
			if (ImprovedFileHandling.saveOperation(chartName, {ext: "json", desc: "JSON File"}, Text, chartData)) {
	...
	{
		return PlayfieldManager.SONG.notes != null ? PlayfieldManager.SONG.notes[curSec] : null;
	}

	function updateNotesRGB()
	{
		PlayfieldManager.SONG.disableNoteRGB = noRGBCheckBox.checked;
	...
	function updateGridVisibility()
	{
		...
		nextGridBg.visible = (curSec < PlayfieldManager.SONG.notes.length - 1 && showNextSection);
	...
	function updateVortexHolds() {
		for (num => key in keysArray) {
			if (_heldNotes[num] != null) {
				...
				note.setSustainLength(note.sustainLength, cachedSectionCrochets[noteSec] / 4, curZoom, reverseScroll);
			}
		}
	}
	...
	function updateWaveform() {
		...
		if(reverseScroll) { // Simply now just flip the waveform sprite :D
			waveformSprite.flipY = true;
			waveformSprite.y = gridBg.y + gridBg.height - waveformSprite.height;
		} else {
			waveformSprite.flipY = false;
			waveformSprite.y = gridBg.y;
		}
