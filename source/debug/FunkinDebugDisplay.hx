...
import backend.util.MemoryUtilBase;
...
class FunkinDebugDisplay extends Sprite
{
  ...
  function buildDebugDisplay(advanced:Bool):Void
  {
    ...
    #else
    final BG_HEIGHT_MULTIPLIER:Float = advanced ? 1 : (MemoryUtilBase.supportsTaskMem()) ? 0.3 : 0.2;
    #end
  ...
  override function __enterFrame(deltaTime:Int):Void
  {
    ...
    #if !html5
    gcMem = MemoryUtilBase.getGCMemory();

    if (gcMem > gcMemPeak) gcMemPeak = gcMem;

    if (MemoryUtilBase.supportsTaskMem())
    {
      taskMem = MemoryUtilBase.getTaskMemory();
