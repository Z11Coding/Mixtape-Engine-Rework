...
class PauseSubState extends MusicBeatSubstate
{
			...
			if (menuItems == difficultyChoices)
			{
				if ((difficultyChoices[curSelected] == '???') || archipelago.APItem.unknownSongs)
				{
					...
					if (FlxG.random.int(1, 20) == 1 && Difficulty.list.length > 1)
					{
						...
						{
						var playlistMeta = PlayState.isPlaylist && PlayState.instance != null ? PlayState.instance.curPlaylist : null;
	...
	/**
	 * Helper function to create a new PlayState with playlist data if applicable.
	 * Checks if current state is PlayState with preloadSong enabled.
	 */
	private static function createPlayStateInstance():PlayState
	{
		var playlistMeta = PlayState.isPlaylist && PlayState.instance != null ? PlayState.instance.curPlaylist : null;
		var songlistMeta = PlayState.isPlaylist && PlayState.instance != null ? PlayState.instance.curSonglist : null;
	...
	/**
	 * Reset the state, creating a new instance if preloadSong is enabled.
	 */
	private static function resetStateWithPreload()
	{
		...
		if (ClientPrefs.data.preloadSong)
		{
			states.LoadingState.loadAndSwitchState(createPlayStateInstance());
		}
		else
		{
			MusicBeatState.switchState(createPlayStateInstance());
		}
	}

	public static function restartSong(noTrans:Bool = false)
	{
		restartSongWithPlaylist(noTrans);
	}

	public static function restartSongWithPlaylist(noTrans:Bool = false)
	{
		PlayState.instance.paused = true; // For lua
		FlxG.sound.music.volume = 0;
		PlayState.instance.vocals.volume = 0;
