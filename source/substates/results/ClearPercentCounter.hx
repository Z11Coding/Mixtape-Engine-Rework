...
class ClearPercentCounter extends FlxTypedSpriteGroup<FlxSprite>
{
  ...
  public function new(x:Float, y:Float, startingNumber:Int = 0, big:Bool = false)
  {
    super(x, y);
    ...
    var clearPercentText:FunkinSprite = FunkinSprite.create(0, 0, 'resultScreen/clearPercent/clearPercentText${big ? 'Small' : ''}');
    clearPercentText.x = big ? 40 : 0;
  ...
  function drawNumbers():Void
  {
    ...
    for (ind => num in seperatedScore)
    {
      ...
      var digitSize = big ? 32 : 72;
      var digitHeightOffset = big ? -4 : 0;
      ...
      if (digitIndex >= members.length)
      {
        ...
        var numb:ClearPercentNumber = new ClearPercentNumber(xPos, yPos, num, variant, this.big);
...
{
  public function new(x:Float, y:Float, digit:Int, variant:Bool, big:Bool)
  {
    super(x, y);

    frames = Paths.getSparrowAtlas('resultScreen/clearPercent/clearPercentNumber${(big ? 'Small' : (variant ? 'Right' : 'Left'))}');
