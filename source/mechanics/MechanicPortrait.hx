...
class MechanicPortrait extends FlxSpriteGroup
{
	...
	public function new(x:Float, y:Float, child:MechanicManager.MechanicData, scrollFactor:FlxPoint = null)
	{
		...
		if (!Assets.exists(Paths.getPath('images/mechanicsMenu/portraits/${image}.png', IMAGE)))
			image = 'blank';
		portrait = new MechanicSprite(x, y);
		portrait.loadGraphic(Paths.image('mechanicsMenu/portraits/${image}'));
		...
		arrowL.loadGraphic(Paths.image('mechanicsMenu/mechanicArr'));
		...
		arrowR.loadGraphic(Paths.image('mechanicsMenu/mechanicArr'));
