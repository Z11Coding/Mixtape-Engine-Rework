...
class GameplaySettingsSubState extends BaseOptionsMenu
{
	public function new()
	{
		...
		option.minValue = 1;
		option.maxValue = ClientPrefs.data.sickWindow - 1;
		addOption(option);
		...
		option.displayFormat = '%vms';
		option.scrollSpeed = 15;
		option.minValue = ClientPrefs.data.perfectWindow + 1;
		option.maxValue = ClientPrefs.data.goodWindow - 1;
		addOption(option);
		...
		option.minValue = ClientPrefs.data.sickWindow + 1;
		option.maxValue = ClientPrefs.data.badWindow - 1;
		addOption(option);
		...
		option.minValue = ClientPrefs.data.goodWindow + 1;
		option.maxValue = 135;
		addOption(option);
		...
		option.minValue = 2;
		option.maxValue = 20;
		if (ClientPrefs.data.safeFrames > 10) option.displayFormat = '%v (Will not save)';
		option.changeValue = 0.1;
		addOption(option);
