...
class VisualsUISubState extends BaseOptionsMenu
{
	...
	public function new()
	{
		...
		if(noteSkins.length > 0)
		{
			...
			if(!noteSkins.contains(ClientPrefs.data.noteSkin))
				ClientPrefs.data.noteSkin = noteSkins[0]; //Reset to default if saved noteskin couldnt be found
		...
		if (noteSplashList.length > 0)
		{
			...
			if (!noteSplashList.contains(ClientPrefs.data.splashType))
				ClientPrefs.data.splashType = noteSplashList[0];
		...
		{
			if (!ratingQuoteList.contains(ClientPrefs.data.rateNameStuff))
				ClientPrefs.data.rateNameStuff = ratingQuoteList[0];
		...
		if (ratingSpriteList.length > 0)
		{
			...
			if (!ratingSpriteList.contains(ClientPrefs.data.ratingType))
				ClientPrefs.data.ratingType = ratingSpriteList[0];
	...
	{
		if(changedMusic) FlxG.sound.playMusic(Paths.music('freakyMenu-' + ClientPrefs.data.daMenuMusic));
		super.destroy();
	}
