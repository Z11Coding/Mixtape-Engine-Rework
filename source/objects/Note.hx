...
class Note extends NoteObject
{
	...
	public static var maxMania:Int = 17; // Base system max for UI purposes
	public static var defaultMania:Int = 3;

	// Dynamic key system - supports infinite keys
	public static function getMaxSupportedMania():Int {
		// Return the highest configured mania in keysShit, or allow infinite extension
		var maxConfigured = 17; // Base system max
		for (key in keysShit.keys()) {
			if (key > maxConfigured) maxConfigured = key;
		}
		return maxConfigured;
	}

	public static function ensureManiaSupport(maniaCount:Int):Void {
		// Ensure we have key configuration for the requested mania
		if (!keysShit.exists(maniaCount)) {
			generateManiaConfig(maniaCount);
		}

		// Extend arrays if needed
		while (ammo.length <= maniaCount) {
			ammo.push(ammo.length + 1);
		}

		while (scales.length <= maniaCount) {
			// Calculate scale based on key count - more keys = smaller notes
			var newScale = Math.max(0.15, 1.0 - (maniaCount * 0.03));
			scales.push(newScale);
		}

		while (lessX.length <= maniaCount) {
			lessX.push(6); // Default lessX value
		}

		while (separator.length <= maniaCount) {
			separator.push(4); // Default separator value
		}

		while (xtra.length <= maniaCount) {
			xtra.push(0); // Default xtra value
		}

		while (posRest.length <= maniaCount) {
			posRest.push(30); // Default posRest value
		}

		while (gridSizes.length <= maniaCount) {
			gridSizes.push(15); // Default grid size for high key counts
		}

		while (xPosButBetter.length <= maniaCount) {
			xPosButBetter.push(2); // Default xPos value
		}

		while (xPosButBetter2.length <= maniaCount) {
			xPosButBetter2.push(1); // Default xPos2 value
		}

		while (xPosButBetterOff.length <= maniaCount) {
			xPosButBetterOff.push(100); // Default offset value
		}

		while (offsets.length <= maniaCount) {
			offsets.push([10, 10]); // Default offsets
		}

		while (noteSplashScales.length <= maniaCount) {
			var newSplashScale = Math.max(0.1, 0.8 - (maniaCount * 0.02));
			noteSplashScales.push(newSplashScale);
		}

		while (pixelScales.length <= maniaCount) {
			var newPixelScale = Math.max(0.2, 1.0 - (maniaCount * 0.03));
			pixelScales.push(newPixelScale);
		}
	}

	private static function generateManiaConfig(maniaCount:Int):Void {
		// Generate configuration for custom key counts
		var letters:Array<String> = [];
		var anims:Array<String> = [];
		var singAnims:Array<String> = [];
		var strumAnims:Array<String> = [];
		var pixelAnimIndex:Array<Int> = [];
		var colArray:Array<Int> = [];

		// Generate letter mappings
		var letterIndex = 0;
		for (i in 0...maniaCount + 1) {
			if (letterIndex < gfxLetter.length) {
				letters.push(gfxLetter[letterIndex]);
			} else {
				// Generate additional letters if needed
				letters.push(String.fromCharCode(65 + (letterIndex % 26))); // A-Z cycling
			}

			// Generate animations based on position
			var animType = i % 4;
			switch (animType) {
				case 0: anims.push("LEFT"); singAnims.push("singLEFT"); strumAnims.push("LEFT");
				case 1: anims.push("DOWN"); singAnims.push("singDOWN"); strumAnims.push("DOWN");
				case 2: anims.push("UP"); singAnims.push("singUP"); strumAnims.push("UP");
				case 3: anims.push("RIGHT"); singAnims.push("singRIGHT"); strumAnims.push("RIGHT");
				default: anims.push("UP"); singAnims.push("singUP"); strumAnims.push("SPACE");
			}

			pixelAnimIndex.push(i % 18); // Cycle through available pixel animations
			colArray.push(i % 4); // Cycle through 4 basic colors
			letterIndex++;
		}

		keysShit.set(maniaCount, [
			"letters" => letters,
			"anims" => anims,
			"singAnims" => singAnims,
			"strumAnims" => strumAnims,
			"pixelAnimIndex" => pixelAnimIndex,
			"colArray" => colArray
		]);
	}
