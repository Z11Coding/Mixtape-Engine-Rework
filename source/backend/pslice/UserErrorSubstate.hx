...
class UserErrorSubstate extends MusicBeatSubstate
{
	...
	public static function collectErrorData(errorMessage:String, callStack:Array<StackItem>):CrashData
	{
		...
		return {
			...
			activeMod: Mods.currentModDirectory
		}
	}
  ...
  {
    return switch(/*ClientPrefs.data.Logging*/"Console"){
	...
	override function update(elapsed:Float)
	{
      ...
      @:privateAccess {
        ScreenshotPlugin.current.destroy();
        ScreenshotPlugin.current = null;
      }
	...
	function printError(error:CrashData)
	{
		...
		printToTrace('MIXTAPE ${states.MainMenuState.mixtapeEngineVersion}$star  (${error.message})');
		textNextY += 35;
		FlxTimer.wait(1 / 24, () ->
		{
			printSpaceToTrace();
			...
			printToTrace('MOD:${error.activeMod.rpad(" ", 10)} PE:${states.MainMenuState.psychEngineVersion.rpad(" ", 5)} SYS:${error.systemName}');
			printSpaceToTrace();
	...
	{
		var test_text = new FlxText(180+FlxG.width/4, textNextY, FlxG.width * 0.71, text.toUpperCase());
