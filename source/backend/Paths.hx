...
class Paths
{
	...
	public static var splashConfigs:Map<String, objects.notes.NoteSplash.NoteSplashConfig> = new Map();
	...
	public static function initSplashConfig(skin:String)
	{
		...
		var config:objects.notes.NoteSplash.NoteSplashConfig = {
			anim: configFile[0],
	...
	public static function url(url:String, ?newMethod:Bool, ?tries:Int, ?soft: Bool):String
	{
		...
		{ if (soft) return null; else
			throw new haxe.Exception('Invalid URL: $url');
		}
	...
	static public function image(key:String, ?parentFolder:String = null, ?allowGPU:Bool = false):FlxGraphic
	{
		key = 'images/$key' + '.png';
