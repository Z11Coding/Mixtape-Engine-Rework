...
class SpiralModifier extends NoteModifier {
	override function getPos(visualDiff:Float, timeDiff:Float, beat:Float, pos:Vector3, column:Int, player:Int, obj:FlxSprite, field:NoteField) {
		...
		if(schmovinSpiralX != 0){
			...
			pos.x += Math.cos(-visualDiff / Conductor.crochet * Math.PI + beat) * radius * schmovinSpiralX;
		}
		if (schmovinSpiralY != 0) {
			...
			pos.y += Math.sin(-visualDiff / Conductor.crochet * Math.PI + beat) * radius * schmovinSpiralY;
		}
		if (schmovinSpiralZ != 0) {
			...
			pos.z += Math.sin(-visualDiff / Conductor.crochet * Math.PI + beat) * radius * schmovinSpiralZ;
		}
