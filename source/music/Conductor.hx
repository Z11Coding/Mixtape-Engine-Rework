...
class Conductor
{
	...
	{
		if (botplay || missedNote) return states.PlayState.instance.ratingsData[0];
		var data:Array<Rating> = states.PlayState.instance.ratingsData; //shortening cuz fuck u
		for(i in 0...Std.int(data.length-1)) //skips last window (Shit)
		{
		...
		return data[Std.int(data.length - 1)];
	}

	public static function judgeNotePsych(arr:Array<Rating>, diff:Float=0):Rating // die
	{
