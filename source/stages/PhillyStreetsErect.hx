...
class PhillyStreetsErect extends backend.BaseStage
    {
        ...
        override function create()
        {
            ...
            {
                var skyImage = Paths.image('stages/weekend/phillyStreets/erect/phillySkybox');
                ...
                var phillySkyline:BGSprite = new BGSprite('stages/weekend/phillyStreets/erect/phillySkyline', -545, -273, 0.2, 0.2);
                add(phillySkyline);
                darkenable.push(phillySkyline);
    
                var phillyForegroundCity:BGSprite = new BGSprite('stages/weekend/phillyStreets/erect/phillyForegroundCity', 625, 94, 0.3, 0.3);
                add(phillyForegroundCity);
                darkenable.push(phillyForegroundCity);
            }
    
            add(mist5);
            var phillyConstruction:BGSprite = new BGSprite('stages/weekend/phillyStreets/erect/phillyConstruction', 1800, 364, 0.7, 1);
            add(phillyConstruction);
            darkenable.push(phillyConstruction);
    
            var phillyHighwayLights:BGSprite = new BGSprite('stages/weekend/phillyStreets/erect/phillyHighwayLights', 284, 305, 1, 1);
            add(phillyHighwayLights);
            darkenable.push(phillyHighwayLights);
    
            if (!ClientPrefs.data.lowQuality)
            {
                var phillyHighwayLightsLightmap:BGSprite = new BGSprite('stages/weekend/phillyStreets/phillyHighwayLights_lightmap', 284, 305, 1, 1);
            ...
            var phillyHighway:BGSprite = new BGSprite('stages/weekend/phillyStreets/erect/phillyHighway', 139, 209, 1, 1);
            add(phillyHighway);
            darkenable.push(phillyHighway);
    
            if (!ClientPrefs.data.lowQuality)
            {
                var phillySmog:BGSprite = new BGSprite('stages/weekend/phillyStreets/phillySmog', -6, 245, 0.8, 1);
                add(phillySmog);
                darkenable.push(phillySmog);
    
                for (i in 0...2)
                {
                    var car:BGSprite = new BGSprite('stages/weekend/phillyStreets/erect/phillyCars', 1200, 818, 0.9, 1, ['car1', 'car2', 'car3', 'car4'], false);
                    add(car);
                    switch (i)
                    {
                        case 0:
                ...
                phillyTraffic = new BGSprite('stages/weekend/phillyStreets/erect/phillyTraffic', 1840, 608, 0.9, 1, ['redtogreen', 'greentored'], false);
                add(phillyTraffic);
                darkenable.push(phillyTraffic);
    
                var phillyTrafficLightmap:BGSprite = new BGSprite('stages/weekend/phillyStreets/erect/phillyTraffic_lightmap', 1840, 608, 0.9, 1);
            ...
            add(mist4);
            //? gradient
            var gray1:BGSprite = new BGSprite('stages/weekend/phillyStreets/erect/greyGradient', 88, 317, 1, 1);
            gray1.alpha = 0.3;
            gray1.blend = ADD;
            add(gray1);

            var gray2:BGSprite = new BGSprite('stages/weekend/phillyStreets/erect/greyGradient', 88, 317, 1, 1);
            gray2.alpha = 0.8;
            gray2.blend = MULTIPLY;
            add(gray2);

            var phillyForeground:BGSprite = new BGSprite('stages/weekend/phillyStreets/erect/phillyForeground', 88, 317, 1, 1);
    ...
    function makeMist(image:String,scrollFac:Float,alpha:Float,velX:Float) {
        var mist = new FlxBackdrop(Paths.image('stages/weekend/phillyStreets/erect/$image'), X);
    ...
    function updateMist(elapsed:Float) {
        ...
        override function openSubState(SubState:FlxSubState) {
            ...
            states.PlayState.instance.subStateClosed.addOnce((sub) ->{
                carSndAmbience.volume = 0.1;
