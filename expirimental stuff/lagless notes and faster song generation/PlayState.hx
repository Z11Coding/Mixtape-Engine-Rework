...
class PlayState extends MusicBeatState
{
	...
	public var unspawnNotesLagless:Array<PreloadedChartNote> = [];
	...
	public var allNotesLagless:Array<PreloadedChartNote> = []; // all notes
	...
	private function generateSong():Void
	{
		...
		var daBpm:Float = Conductor.bpm;
		var swagNoteLagless:PreloadedChartNote;
		var sustainNoteLagless:PreloadedChartNote;
		var swagNote:Note;
		...
		for (section in sectionsData)
		{
			...
			for (i in 0...section.sectionNotes.length)
			{
				...
				if (i != 0) {
					...
					for (evilNote in allNotesLagless) {
						...
						if (matches && Math.abs(spawnTime - evilNote.strumTime) < flixel.math.FlxMath.EPSILON) {
							...
							if (evilNote.tail.length > 0)
								...
								{
									allNotesLagless.remove(tail);
									if (playfield != null) playfield.unqueue(tail);
								}
							allNotesLagless.remove(evilNote);
				...
				if (allNotesLagless.length > 0)
					oldNote = Note.quickMakeNote(allNotesLagless[Std.int(allNotesLagless.length - 1)]);
				else
					oldNote = null;

				var isAlt: Bool = section.altAnim && !gottaHitNote;
				swagNoteLagless = cast {
					strumTime: spawnTime,
					noteData: noteColumn,
					column: noteColumn,
					prevNote: oldNote,
					...
					sustainLength: songNotes[2] <= curStepCrochet ? songNotes[2] : (holdLength + 1) * curStepCrochet,
					noAnimation: false,
					...
					blockHit: false,
					lowPriority: false,
					row: Conductor.secsToRow(spawnTime),
					holdType: ((songNotes[2] <= curStepCrochet ? songNotes[2] : (holdLength + 1) * curStepCrochet) > 0 ? HEAD : TAP),
					isParent: true,
					multSpeed: 1,
					noteIndex: Std.int(allNotesLagless.length),
					field: null,
					fieldIndex: -1,
					inEditor: false,
					spawned: false,
				...
				if(rowArray[swagNoteLagless.row]==null)
					rowArray[swagNoteLagless.row]=[];
				rowArray[swagNoteLagless.row].push(swagNoteLagless);*/
				if (!swagNoteLagless.mustPress)
				{
					if (AIPlayMap != null && AIPlayMap.length != 0 && [sectionsData.indexOf(section)] != null)
					{
						swagNoteLagless.AIStrumTime = AIPlayMap[sectionsData.indexOf(section)][section.sectionNotes.indexOf(songNotes)];
						if (Math.abs(swagNoteLagless.AIStrumTime) > Conductor.safeZoneOffset)
							swagNoteLagless.ignoreNote = swagNoteLagless.AIMiss = true;
					}
				}

				var playfield:PlayField = swagNoteLagless.field;

				if (playfield == null && playfields.length > 0) {
					if (swagNoteLagless.fieldIndex == -1)
						swagNoteLagless.fieldIndex = swagNoteLagless.mustPress ? 0 : 1;

					if (playfields.members[swagNoteLagless.fieldIndex] != null) {
						playfield = playfields.members[swagNoteLagless.fieldIndex];
						swagNoteLagless.field = playfield;
					}
				}

				playfield = swagNoteLagless.field;
				swagNoteLagless.fieldIndex = playfield.modNumber;
				
				if (playfield != null)
				{
					playfield.queue(swagNoteLagless); // queues the note to be spawned
					allNotesLagless.push(swagNoteLagless); // just for the sake of convenience
				}
				...
				var spot = 0;
				final roundSus:Int = Math.round(swagNoteLagless.sustainLength / Conductor.stepCrochet) -1;
				if (roundSus > 0)
				{
					for (susNote in 0...roundSus)
					{
						...
						sustainNoteLagless = cast {
							strumTime: spawnTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet),
							noteData: noteColumn,
							column: noteColumn,
							prevNote: swagNoteLagless,
							mustPress: gottaHitNote,
							noteType: swagNoteLagless.noteType,
							animSuffix: swagNoteLagless.animSuffix,
							noAnimation: false,
							noMissAnimation: false,
							gfNote: swagNoteLagless.gfNote,
							isSustainNote: true,
							hitHealth: 0.02,
							missHealth: 0.1,
							hitCausesMiss: false,
							...
							lowPriority: false,
							row: Conductor.secsToRow(spawnTime),
							holdType: (roundSus > 0 ? PART : END),
							isParent: false,
							multSpeed: 1,
							noteIndex: swagNoteLagless.noteIndex,
							field: swagNoteLagless.fieldIndex,
							fieldIndex: swagNoteLagless.fieldIndex,
							spotInLine: spot,
							inEditor: false,
							spawned: false,
							parent: swagNoteLagless,
							template: this
						}

						swagNoteLagless.tail.push(sustainNoteLagless);
						swagNoteLagless.unhitTail.push(sustainNoteLagless);
						swagNoteLagless.childs.push(sustainNoteLagless);
						sustainNoteLagless.parent = swagNoteLagless;

						if (playfield != null)
						{
							//playfield.queue(sustainNoteLagless); // queues the note to be spawned
							//allNotesLagless.push(sustainNoteLagless); // just for the sake of convenience
						}
						
						//playfield.queue(sustainNoteLagless);
						//allNotesLagless.push(sustainNoteLagless);

						spot++;

						/*var sustainNote:Note = new Note(spawnTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet), noteColumn, swagNote, true);
						...
						swagNoteLagless.tail.push(sustainNoteLagless);
						swagNoteLagless.unhitTail.push(sustainNoteLagless);
						sustainNote.tail.push(sustainNote);
						sustainNote.unhitTail.push(sustainNote);
						...
						sustainNote.parent = swagNote;
						swagNoteLagless.parent = swagNoteLagless;
						swagNoteLagless.childs.push(swagNoteLagless);
						sustainNote.childs.push(sustainNote);
		...
		allNotesLagless.sort(sortByTime);
		
		unspawnNotesLagless = allNotesLagless;
	...
	public function initPlayfield(field:PlayField){
		...
		field.noteRemoved.add((note:Note, preloadNote:PreloadedChartNote, field:PlayField) -> {
			allNotesLagless.remove(preloadNote);
			unspawnNotesLagless.remove(preloadNote);
			notes.remove(note);
		});

		field.noteMissed.add((daNote:Note, preloadNote:PreloadedChartNote, field:PlayField) -> {
			if (field.isPlayer && !field.autoPlayed && !daNote.ignoreNote && !endingSong && (daNote.tooLate || !daNote.wasGoodHit))
				noteMiss(daNote, field);

		});

		field.noteSpawned.add((dunceNote:Note, preloadNote:PreloadedChartNote, field:PlayField) -> {
			...
			curNote = Note.quickMakeNote(preloadNote);
			notes.add(curNote);
			
			var index:Int = unspawnNotesLagless.indexOf(unspawnNotesLagless[dunceNote.noteIndex]);
			unspawnNotesLagless.splice(index, 1);
		...
		field.holdDropped.add((daNote:Note, preloadNote:PreloadedChartNote, field:PlayField) -> {
			if (!field.isPlayer)return;
		});

		field.holdFinished.add((daNote:Note, preloadNote:PreloadedChartNote, field:PlayField) -> {
			if (!field.isPlayer)return;
		}); 
	...
	inline function stepHold(note:Note, preloadNote:PreloadedChartNote, field:PlayField)
	{
	...
	inline function pressHold(note:Note, preloadNote:PreloadedChartNote, field:PlayField) {
		callOnScripts("onHoldPress", [note, field]);
	}

	inline function releaseHold(note:Note, preloadNote:PreloadedChartNote, field:PlayField):Void {
	...
	override public function update(elapsed:Float)
	{
		...
		if (allowSkip)
		{
			if (ClientPrefs.data.skipMode == 'First Note') {
				var daNote:PreloadedChartNote = allNotesLagless[0];
	...
	public function opponentNoteHit(note:Note, preloadNote:PreloadedChartNote, field:PlayField):Void
	{
		var result:Dynamic;
		if (opponentmode)
		{
	...
	public function goodNoteHit(note:Note, preloadNote:PreloadedChartNote, field:PlayField):Void
	{
