...
class APNote extends objects.Note {
    ...
    var curNote:objects.Note;
    public static function replaceInQueue(notes:Array<Array<objects.Note.PreloadedChartNote>>, locations:Array<Int>, ?ignoreNonEmptyNoteType:Bool = true) {
        var newNotes:Array<APNote> = [];
        var flatNotes:Array<{lane:Int, index:Int, note:objects.Note.PreloadedChartNote}> = [];
        var isAprilFools:Bool = yutautil.AprilFools.allowAF;
