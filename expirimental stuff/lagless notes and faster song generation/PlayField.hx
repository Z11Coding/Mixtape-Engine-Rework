...
typedef NoteCallback = (Note, PreloadedChartNote, PlayField) -> Void;
class PlayField extends FlxTypedGroup<FlxBasic>
{
	...
	public var noteQueue:Array<Array<PreloadedChartNote>> = [[], [], [], [], [], [], [], [],[], [], [], [],[], [], [], [], [], []]; // unspawned notes
...
//		splash.handleRendering = false;
	...
	// unqueues a note
	public function unqueue(note:PreloadedChartNote)
	{
		if (noteQueue[note.column] == null)
			noteQueue[note.column] = [];
	...
	// destroys a note
	public function removeNote(daNote:Note, ?preloadNote:PreloadedChartNote, ?killTail:Bool = false){
		daNote.active = false;
		daNote.visible = false;
		daNote.kill();

		noteRemoved.dispatch(daNote, preloadNote, this);
		...
		if (daNote.unhitTail.length > 0)
			while (daNote.unhitTail.length > 0)
				removeNote(daNote.unhitTail.shift(), daNote.unhitTail.shift().template);
	...
	// spawns a note
	var curNote:Note;
	public function spawnNote(note:PreloadedChartNote){
		curNote = Note.quickMakeNote(note);
		...
		if (spawnedByData[note.column] != null) {
			...
			spawnedByData[note.column].push(curNote);
		} else {
		...
		if(note.holdType == HEAD || note.holdType == TAP) {
			...
			if (tapsByData[note.column] != null) {
				...
				tapsByData[note.column].push(curNote);
			} else {
		...
		noteSpawned.dispatch(curNote, curNote.template, this);
		// trace("Adding note to spawnedNotes.");
		spawnedNotes.push(curNote);
		curNote.handleRendering = false;
	...
	public function getAllNotes(?dir:Int){
		...
		if(dir==null){
			for(queue in noteQueue){
				for(note in queue)
					arr.push(Note.quickMakeNote(note));
	...
	public function hasNote(note:Note)
		return spawnedNotes.contains(note) || noteQueue[note.column]!=null && noteQueue[note.column].contains(note.template);
			...
			switch (ClientPrefs.data.inputSystem)
			{
				case "Native":
					...
					while (noteList.length > 0)
					{
						...
						if (!note.blockHit || !note.forceBlockHit) noteHitCallback(note, note.template, this);
						return note;
					}
				case "Native-old":
					if(data > keyCount || data < 0)return null;
					...
					while (noteList.length > 0)
					{
						...
						if (note.wasGoodHit && note.holdingTime < note.sustainLength)
							...
							if (note.wasGoodHit)
								continue;
							if (!note.blockHit) noteHitCallback(note, note.template, this);
							return note;
						}
					}
					return recentHold;
				case 'Rhythm':
					...
					while (noteList.length > 0)
					{
						...
						{
							noteHitCallback(note, note.template, this);
							return note;
						}
					}
					...
					for (note in spawnedNotes)
					{
						if (note.column == data && note.isSustainNote && note.wasGoodHit && !note.tooLate && note.holdingTime >= note.sustainLength)
						{
							...
							if (inControl) noteMissed.dispatch(note, note.template, this);
							return note;
						}
					}
					...
					if (noteList.length > 0)
					{
						for (epicNote in noteList)
						{
							for (doubleNote in pressNotes)
							{
								...
								{
									removeNote(doubleNote, doubleNote.template);
								}
								else
									notesStopped = true;
							}
					...
					if (hittableNotes.length != 0)
					{
						...
						if (hittableNotes.length > 1)
						{
							for (shitNote in hittableNotes)
							{
								...
								{
									noteHitCallback(shitNote, shitNote.template, this);
									return shitNote;
								}
						...
						noteHitCallback(daNote, daNote.template, this);
					}
					...
					if (possibleNotes.length > 0 && !dontCheck || possibleNotes.length > 0 && ClientPrefs.data.noAntimash)
					{
						...
						for (coolNote in possibleNotes)
						{
							if (keysPressed[coolNote.noteData])
							{
								...
								{
									noteHitCallback(coolNote.prevNote, coolNote.prevNote.template, this);
								...
								noteHitCallback(coolNote, coolNote.template, this);
								return coolNote;
							}
						}
					}
				...
				case "Andromeda (legacy)":
					...
					if(noteList.length>0){
						...
						{
							if (!hitNote.blockHit) noteHitCallback(hitNote, hitNote.template, this);
							return hitNote;
						}
					}else{
						if(!ClientPrefs.data.ghostTapping)
	...
	override public function update(elapsed:Float){
					...
					if(!daNote.tooLate && daNote.wasGoodHit){
                        ...
                        if(wasHeld != isHeld){
                            if(isHeld){
                                ...
                                if(holdPressCallback != null)
                                    holdPressCallback(daNote, daNote.template, this);
                            }else if(holdReleaseCallback!=null) {
                                holdReleaseCallback(daNote, daNote.template, this);
								trace("Hold Released!");
							}
                        }
						...
						if(oldSteps < currentSteps)
							if(holdStepCallback != null)
								holdStepCallback(daNote, daNote.template, this);
						...
						for (tail in daNote.unhitTail)
						{
							if ((tail.strumTime - 25) <= Conductor.songPosition && !tail.wasGoodHit && !tail.tooLate){
								noteHitCallback(tail, tail.template, this);
								trace("hitting tail hold");
							}
						}
						...
						{
							trace("finished hold");
							holdFinished.dispatch(daNote, daNote.template, this);
			...
			else
			{
				...
				if (daNote.tooLate && daNote.active && !daNote.causedMiss)
				{
					...
					if (!daNote.ignoreNote && (daNote.tooLate || !daNote.wasGoodHit) && inControl)
						noteMissed.dispatch(daNote, daNote.template, this);
				}
		...
		for(note in garbage) removeNote(note, note.template);
	...
	public function forEachQueuedNote(callback:Note->Void)
	{
		for(column in noteQueue){
			var i:Int = 0;
			var note:PreloadedChartNote = null;

			while (i < column.length)
			{
				note = column[i++];
				var curNote:Note = Note.quickMakeNote(note);
				if (curNote != null && curNote.exists && curNote.alive)
					callback(curNote);
			}
		}
	}
				...
				for (nIdx in 1...column.length)
				{
					...
					if (Math.abs(last.strumTime - current.strumTime) <= Conductor.stepCrochet / (192 / 16))
					{
						if (last.sustainLength < current.sustainLength) { // keep the longer hold
							// removeNote(null, last);
							PlayState.instance.allNotesLagless.remove(last);
							PlayState.instance.unspawnNotesLagless.remove(last);
						}
						else
						{
							// removeNote(null, current);
							PlayState.instance.allNotesLagless.remove(current);
							PlayState.instance.unspawnNotesLagless.remove(current); // mark to delete after, cant delete here because otherwise it'd fuck w/ stuff
						}
					}
				}
			}
